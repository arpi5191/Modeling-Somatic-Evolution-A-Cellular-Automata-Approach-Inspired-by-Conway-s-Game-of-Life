...
def simulate_population_with_lonely_mutation(initial_board, num_gens=10000, mutation_rate=0.0, mutation_magnitude=0.0):
    ...
    Returns:
    int: Final average lonely threshold.
    """
    ...
    for gen in range(num_gens):
                    ...
                    if num_neighbors == cell_born:
                        ...
                        if live_neighbors:
                            ...
                            if random.random() < mutation_rate:
                                # Randomly select a threshold to mutate
                                threshold = random.choice(['lonely', 'born', 'crowded'])
                                # Multiply the mutation magnitude with a value from a normal distribution and negate
                                delta = -abs(mutation_magnitude * np.random.normal(0, 1))
        ...
        # Check if the board from the last generation is the same as this generation
        if np.array_equal(board, new_board):
            # Return the average lonely threshold if steady state was reached
    ...
    # Return the final average lonely threshold if steady state wasn't reached
