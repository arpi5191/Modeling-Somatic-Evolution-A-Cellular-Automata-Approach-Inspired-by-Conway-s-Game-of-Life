...
def play_game_of_life_with_mutation(initial_board, num_gens=10000, mutation_rate=0.0, mutation_magnitude=0.0):
    """
    Simulates the Game of Life with threshold mutations, returning the number of generations until steady state is reached.
...
def simulate_population_with_mutation(initial_board, num_gens=10000, mutation_rate=0.0, mutation_magnitude=0.0):
    """
    Simulates the Game of Life applying thresholds with mutation over the specified number of generations, returning the board's population.
...
def simulate_population_with_lonely_mutation(initial_board, num_gens=10000, mutation_rate=0.0, mutation_magnitude=0.0):
    """
    Simulates the Game of Life applying thresholds with mutation over the specified number of generations, returning the final average lonely
    threshold.
    ...
    Returns:
    float: Final average lonely threshold.
    """
    ...
    # Initialize the lonely, born, and crowded boards based on the number of rows and columns
    rows, cols = board.shape
    lonely = np.full((rows, cols), 2.0)  # Using float instead of int for thresholds
    born = np.full((rows, cols), 3.0)
    crowded = np.full((rows, cols), 3.0)

    # Track previous population to check for steady state
    prev_populations = []
    ...
    for gen in range(num_gens):
                ...
                # Otherwise, check if the cell is dead
                    ...
                    if num_neighbors == cell_born:
                        ...
                        if live_neighbors:
                            ...
                            # Apply mutation with specified probability
                            if random.random() < mutation_rate:
                                # Randomly select a threshold to mutate (paper showed lonely was critical)
                                threshold = random.choice(['lonely', 'born', 'crowded'])
                                
                                # Generate mutation: normally distributed random number * mutation magnitude
                                delta = mutation_magnitude * np.random.normal(0, 1)
        ...
        # Add current population to history (keep last 15)
        prev_populations.append(np.sum(new_board))
        if len(prev_populations) > 15:
            prev_populations.pop(0)
            
        # If populations are stable for 15 generations, consider it steady state
        if len(prev_populations) == 15 and len(set(prev_populations)) == 1:
            return np.mean(new_lonely)
